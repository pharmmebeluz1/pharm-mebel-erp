...
def require_login():
    ...
    public_endpoints = {
        ...
        "login", "admin_setup", "static", "public_track", "public_track_rating", "order_qr",
...
@app.route("/kuzatuv/<token>/baho", methods=["POST"])
def public_track_rating(token):
    """Faqat mijozning maxfiy kuzatuv havolasi orqali yakunlangan buyurtmaga baho qabul qiladi."""
    c = get_db()
    order = c.execute("SELECT * FROM buyurtmalar WHERE tracking_token=?", (token,)).fetchone()
    if not order:
        c.close()
    ...
    oid = int(order["id"])
    stages = c.execute(
        "SELECT bajarildi FROM buyurtma_bosqichlari WHERE buyurtma_id=? ORDER BY id",
        (oid,)
    ).fetchall()
    done = sum(int(x["bajarildi"] or 0) for x in stages)
    ready = (
        str(order["holat"] or "") in {"Tayyor", "Yetkazishga tayyor", "Yetkazildi", "Yopildi", "Yakunlandi"}
        or (bool(stages) and done == len(stages))
    )
    if not ready:
        c.close()
        return redirect(url_for(
            "public_track", token=token,
            xabar="Baholash buyurtma yakunlangandan keyin ochiladi."
        ))

    existing = c.execute(
        "SELECT id FROM baholar WHERE buyurtma_id=? ORDER BY id DESC LIMIT 1",
        (oid,)
    ).fetchone()
    if existing:
        c.close()
        return redirect(url_for(
            "public_track", token=token,
            xabar="Bu buyurtmaga avval baho berilgan. Rahmat!"
        ))
    ...
    if stars < 1 or stars > 5:
        ...
        return redirect(url_for(
            "public_track", token=token,
            xabar="Iltimos, 1 dan 5 gacha yulduzcha tanlang."
        ))

    selected = [str(x).strip() for x in request.form.getlist("tags") if str(x).strip()]
    allowed = {
        key: label for key, label, _icon in
        (CUSTOMER_GOOD_TAGS if stars >= 4 else CUSTOMER_BAD_TAGS)
    }
    selected_labels = [allowed[x] for x in selected if x in allowed]
    comment = (request.form.get("comment") or "").strip()[:500]
    employee_score = stars * 20

    note_parts = []
    if selected_labels:
        note_parts.append("Teglar: " + ", ".join(selected_labels))
    note_parts.append(f"Xodim balli: {employee_score}/100")
    if comment:
    ...
    return redirect(url_for(
        "public_track", token=token,
        xabar=f"Rahmat! {stars} yulduzli bahoyingiz qabul qilindi."
    ))
...
def public_track(token):
    ...
    ready=str(order["holat"] or "") in {"Tayyor","Yetkazishga tayyor","Yetkazildi","Yopildi","Yakunlandi"} or pct>=100
    deadline=_order_deadline(order)
    ...
    *{box-sizing:border-box}body{margin:0;font-family:Arial,sans-serif;background:linear-gradient(180deg,#e8f4ff 0,#f4f7fa 260px);color:var(--ink)}
    ...
    .rating-card{text-align:center;padding:24px 18px}.rating-kicker{font-size:12px;font-weight:900;color:#0757a6;letter-spacing:.8px;text-transform:uppercase}.rating-title{font-size:28px;line-height:1.14;margin:8px auto 6px;max-width:650px}.rating-sub{color:var(--muted);margin:0 auto 16px;max-width:620px}.stars{display:flex;justify-content:center;gap:7px;margin:12px 0 20px}.star-btn{border:0;background:transparent;color:#dbe2ea;font-size:48px;line-height:1;padding:2px;cursor:pointer;filter:none}.star-btn.on{color:#facc15;transform:translateY(-1px)}.rating-question{font-size:19px;font-weight:900;margin:8px 0 14px}.tag-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:9px;margin:0 auto 16px;max-width:760px}.tag-chip{position:relative;display:flex;align-items:center;gap:9px;text-align:left;padding:12px;border:1px solid #dbe5ee;border-radius:14px;background:#f8fafc;cursor:pointer;min-height:58px}.tag-chip:hover{border-color:#93c5fd;background:#eff6ff}.tag-chip input{position:absolute;opacity:0;pointer-events:none}.tag-chip:has(input:checked){border-color:#60a5fa;background:#eff6ff;box-shadow:0 0 0 2px #dbeafe}.tag-icon{width:30px;height:30px;border-radius:10px;display:grid;place-items:center;background:#e8f5ff;color:#0757a6;font-weight:900;flex:none}.tag-group{display:none}.tag-group.show{display:grid}.rating-comment{width:100%;min-height:96px;resize:vertical;border:1px solid #cbd5e1;border-radius:13px;padding:12px;font:inherit;outline:none}.rating-comment:focus{border-color:#60a5fa;box-shadow:0 0 0 3px #dbeafe}.rating-counter{text-align:right;color:var(--muted);font-size:12px;margin-top:4px}.rating-submit{width:100%;max-width:600px;margin-top:15px;border:0;border-radius:999px;padding:14px 20px;background:linear-gradient(90deg,#0757a6,#1688df);color:#fff;font-weight:900;font-size:16px;cursor:pointer}.rating-submit:disabled{opacity:.5;cursor:not-allowed}.rating-done{background:#ecfdf5;border:1px solid #86efac}.rating-done .rating-title{color:#166534}.support-center{margin-top:16px;padding-top:15px;border-top:1px solid #e2e8f0;display:flex;gap:8px;justify-content:center;flex-wrap:wrap}.support-center .btn{border-radius:999px}
    @media(max-width:700px){.rating-title{font-size:23px}.star-btn{font-size:42px}.tag-grid{grid-template-columns:1fr 1fr}.rating-card{padding:20px 14px}}
    @media(max-width:430px){.tag-grid{grid-template-columns:1fr}.star-btn{font-size:38px;gap:3px}}
    ...
    <main class="wrap">
      ...
      {% if ready %}
        {% if rating %}
        <section class="card rating-card rating-done" style="margin-top:14px">
          <div class="rating-kicker">Mijoz bahosi</div>
          <div class="rating-title">Rahmat! Bahoyingiz saqlangan ⭐</div>
          <p class="rating-sub">Siz ushbu buyurtmaga <b>{{rating['umumiy']}} / 5</b> yulduz berdingiz. Fikringiz xizmat sifatini yaxshilashga yordam beradi.</p>
          {% if contact or whatsapp %}<div class="support-center"><span class="rating-sub" style="margin:8px 4px 0 0">Qo‘llab-quvvatlash markazi:</span>{% if contact %}<a class="btn" href="tel:{{contact}}">📞 Qo‘ng‘iroq</a>{% endif %}{% if whatsapp %}<a class="btn green" href="{{whatsapp}}" target="_blank">💬 Xabar yozish</a>{% endif %}</div>{% endif %}
        </section>
        {% else %}
        <section class="card rating-card" style="margin-top:14px" id="customerRating">
          <div class="rating-kicker">Mijoz bahosi</div>
          <div class="rating-title" id="ratingTitle">Buyurtmangizni baholang</div>
          <p class="rating-sub">Bahoyingiz mas’ul xodim reytingiga qo‘shiladi. Mas’ul xodim: <b>{{o['masul_xodim'] or 'Belgilanmagan'}}</b></p>
          <form method="post" action="/kuzatuv/{{o['tracking_token']}}/baho" id="ratingForm">
            <input type="hidden" name="csrf_token" value="{{csrf_token()}}">
            <input type="hidden" name="stars" id="ratingStars" value="">
            <div class="stars" aria-label="1 dan 5 gacha baho">
              <button type="button" class="star-btn" data-star="1" aria-label="1 yulduz">★</button>
              <button type="button" class="star-btn" data-star="2" aria-label="2 yulduz">★</button>
              <button type="button" class="star-btn" data-star="3" aria-label="3 yulduz">★</button>
              <button type="button" class="star-btn" data-star="4" aria-label="4 yulduz">★</button>
              <button type="button" class="star-btn" data-star="5" aria-label="5 yulduz">★</button>
            </div>
            <div class="rating-question" id="ratingQuestion">Yulduzchani tanlang</div>
            <div class="tag-grid tag-group" id="goodTags">
              {% for key,label,icon in good_tags %}<label class="tag-chip"><input type="checkbox" name="tags" value="{{key}}"><span class="tag-icon">{{icon}}</span><span>{{label}}</span></label>{% endfor %}
            </div>
            <div class="tag-grid tag-group" id="badTags">
              {% for key,label,icon in bad_tags %}<label class="tag-chip"><input type="checkbox" name="tags" value="{{key}}"><span class="tag-icon">{{icon}}</span><span>{{label}}</span></label>{% endfor %}
            </div>
            <textarea class="rating-comment" name="comment" id="ratingComment" maxlength="500" placeholder="Sharh qo‘shing yoki taklif qoldiring"></textarea>
            <div class="rating-counter"><span id="ratingCount">0</span>/500</div>
            <button class="rating-submit" type="submit" id="ratingSubmit" disabled>Jo‘natish</button>
          </form>
          {% if contact or whatsapp %}<div class="support-center"><span class="rating-sub" style="margin:8px 4px 0 0">Qo‘llab-quvvatlash markazi:</span>{% if contact %}<a class="btn" href="tel:{{contact}}">📞 Qo‘ng‘iroq</a>{% endif %}{% if whatsapp %}<a class="btn green" href="{{whatsapp}}" target="_blank">💬 Xabar yozish</a>{% endif %}</div>{% endif %}
        </section>
        {% endif %}
    ...
    <script>
    (function(){
      const el=document.getElementById('countdown'),deadline={{deadline_ms}},ready={{'true' if ready else 'false'}},wait={{countdown_wait|tojson}};
      function pad(n){return String(n).padStart(2,'0')}
      function tick(){if(!el)return;if(ready){el.textContent='Buyurtmangiz tayyor';el.className='countdown green';return}if(!deadline){el.textContent=wait||'Muddat belgilanmagan';el.className='countdown yellow';return}const diff=deadline-Date.now(),abs=Math.abs(diff),days=Math.floor(abs/86400000),hours=Math.floor(abs%86400000/3600000),mins=Math.floor(abs%3600000/60000),secs=Math.floor(abs%60000/1000),value=`${days} kun ${pad(hours)}:${pad(mins)}:${pad(secs)}`;if(diff<0){el.textContent=value+' kechikdi';el.className='countdown red'}else if(diff<86400000){el.textContent=value+' qoldi';el.className='countdown orange'}else if(diff<3*86400000){el.textContent=value+' qoldi';el.className='countdown yellow'}else{el.textContent=value+' qoldi';el.className='countdown green'}}
      tick();setInterval(tick,1000);

      const form=document.getElementById('ratingForm');
      if(!form)return;
      const stars=[...form.querySelectorAll('.star-btn')],value=document.getElementById('ratingStars'),
            good=document.getElementById('goodTags'),bad=document.getElementById('badTags'),
            title=document.getElementById('ratingTitle'),question=document.getElementById('ratingQuestion'),
            submit=document.getElementById('ratingSubmit'),comment=document.getElementById('ratingComment'),
            count=document.getElementById('ratingCount');
      function stopAutoRefresh(){
        const meta=document.querySelector('meta[http-equiv="refresh" i]');
        if(meta)meta.remove();
      }
      function choose(n){
        stopAutoRefresh();value.value=String(n);
        stars.forEach((s,i)=>s.classList.toggle('on',i<n));
        form.querySelectorAll('input[name="tags"]').forEach(x=>x.checked=false);
        if(n>=4){
          title.textContent='Ko‘nglingiz nimadan to‘ldi?';
          question.textContent='Sizga nimasi ko‘proq yoqdi?';
          good.classList.add('show');bad.classList.remove('show');
        }else{
          title.textContent='Biz nimani yaxshilashimiz mumkin?';
          question.textContent='Qaysi jihatni yaxshilashimiz kerak?';
          bad.classList.add('show');good.classList.remove('show');
        }
        submit.disabled=false;
      }
      stars.forEach(s=>s.addEventListener('click',()=>choose(Number(s.dataset.star))));
      form.addEventListener('change',stopAutoRefresh);
      comment.addEventListener('focus',stopAutoRefresh);
      comment.addEventListener('input',()=>{count.textContent=String(comment.value.length)});
      form.addEventListener('submit',e=>{if(!value.value){e.preventDefault();alert('Iltimos, yulduzcha tanlang.')}});
    })();
    </script></body></html>"""
    return render_template_string(html,o=order,stages=stages,pct=pct,current_stage=current_stage,
        ...
        delivery=delivery,media=media,approvals=approvals,rating=rating,
        good_tags=CUSTOMER_GOOD_TAGS,bad_tags=CUSTOMER_BAD_TAGS,
