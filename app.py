...
def save_job(meta: dict[str, Any], parts: list[Part], plans: list[SheetPlan]) -> tuple[int, str]:
    ...
    cur = conn.execute(
        """INSERT INTO kroy_jobs(
               ...
               optimization_mode,tested_variants,best_variant
           ) VALUES(?,?,?,?,?,?,?,?,?,'Ishchiga yuborildi',?,?,?,?,?)""",
        (
            ...
            meta.get("optimization_mode", "large"), int(meta.get("tested_variants", 0)), meta.get("best_variant", ""),
        ),
    )
...
<body><header><div class="top"><div><div class="brand">Mebel360° - Konstruktor Kroy</div><div class="sub">Kroy, kromka, ishchi topshirig'i va A4 chop etish</div></div><div class="top-actions no-print">{% if session.get('admin_user_id') %}<span class="small">{{ session.get('admin_username','Admin') }}</span><a class="btn light" href="{{ url_for('constructor') }}">Konstruktor</a><form method="post" action="{{ url_for('logout') }}"><button class="btn2" type="submit">Chiqish</button></form>{% else %}<a class="btn light" href="{{ url_for('login') }}">Kirish</a>{% endif %}</div></div></header>
...
<div class="grid">
  <div>
    <div class="panel">
      <h3>1. Buyurtma va list</h3>
      <form method="post" action="{{ url_for('job_create') }}" id="jobForm">
    ...
    <div class="panel">
      <h3>2. Eski kroy DBF fayli</h3>
      <form method="post" action="{{ url_for('dbf_import') }}" enctype="multipart/form-data">
        <label>2D-PLACE `.DBF` fayli<input type="file" name="dbf" accept=".dbf" required></label>
        <button type="submit" class="btn2">DBFdan detallarni yuklash</button>
      </form>
      <p class="muted">NAME, LENGTH, WIDTH, MANY va 4 ta BORDER ustuni avtomatik olinadi.</p>
    </div>
  </div>
  <div>
    <div class="panel">
      <h3>3. Detallar va kromka</h3>
      <div class="row"><label>Detal nomi<input id="pName" placeholder="Bokovina"></label><label>Soni<input id="pQty" type="number" min="1" value="1" oninput="renderEntryEdges()"></label></div>
      <div class="row"><label>Uzunligi, mm<input id="pLength" type="number" min="1" placeholder="2100" oninput="renderEntryEdges()"></label><label>Eni, mm<input id="pWidth" type="number" min="1" placeholder="300" oninput="renderEntryEdges()"></label></div>
...
function addPart(){
 ...
 let name=nameEl.value.trim(),length=Number(lenEl.value),width=Number(widEl.value),qty=Number(qtyEl.value||1);if(!name||length<=0||width<=0){alert("Detal nomi va o'lchamini to'g'ri yozing");return;}parts.push({uid:Math.random().toString(36).slice(2),name,length,width,qty,rotate:rotEl.checked,edge_left:entryEdges.left,edge_right:entryEdges.right,edge_top:entryEdges.top,edge_bottom:entryEdges.bottom});nameEl.value='';lenEl.value='';widEl.value='';qtyEl.value=1;entryEdges={top:false,bottom:false,left:false,right:false};renderEntryEdges();renderParts();}
...
function prepareSubmit(){if(!parts.length){alert('Kamida bitta detal kiriting');return false;}document.getElementById('partsJson').value=JSON.stringify(parts);return true;}renderRotateStatus();renderEntryEdges();renderParts();
</script>
"""
...
<div class="panel">
  ...
  <div class="statusline"><span class="stat">{{ sheets|length }} ta list</span><span class="stat">{{ total_parts }} ta detal</span><span class="stat">Jami kromka ×1.1: {{ '%.2f'|format(edge_m) }} m</span><span class="stat">Kroy usuli: {{ opt_label }}</span><span class="stat">Tekshirildi: {{ job.tested_variants }} variant</span><span class="stat">Holat: {{ job.status }}</span></div>
  <div class="actions no-print"><a class="btn ok" href="{{ worker_url }}" target="_blank">Ishchi oynasini ochish</a><button class="btn2" onclick="navigator.clipboard.writeText('{{ worker_url }}').then(()=>alert('Havola nusxalandi'))">Havolani nusxalash</button><a class="btn warn" href="{{ url_for('job_pdf',job_id=job.id) }}" target="_blank">A4 PDF (2 list/bet)</a></div>
</div>
<div class="sheet-grid">{% for row,plan,svg in sheets %}<div class="sheet-card"><div class="sheet-head"><div><div class="sheet-title">List {{ plan.number }} · {{ plan.width }}×{{ plan.height }} mm</div><div class="muted">{{ plan.placements|length }} detal · Foydalanish {{ usage(plan) }}% · <span class="offcut-stat">{{ offcut(plan) }}</span> · {{ opt_label }} / {{ job.tested_variants }} variant</div></div><span class="badge">{{ 'Kesildi' if row.cut_done else 'Kutilmoqda' }}</span></div>{{ svg|safe }}<div class="legend"><b>O'lcham yozuvi atrofidagi qizil qisqa chiziq = kromka.</b> Pushti maydon = qoldiq. <b>GUL ↕</b> = detalni aylantirib bo'lmaydi. T=tepa, P=past, Ch=chap, O=o'ng.</div></div>{% endfor %}</div>
"""
...
def job_create() -> Response:
    try:
        ...
        parts = [
            Part(
                uid=str(p.get("uid") or secrets.token_hex(4)), name=str(p["name"]).strip()[:80], length=int(p["length"]), width=int(p["width"]),
                qty=max(1, min(999, int(p.get("qty", 1)))), rotate=bool(p.get("rotate", True)), edge_left=bool(p.get("edge_left")),
                edge_right=bool(p.get("edge_right")), edge_top=bool(p.get("edge_top")), edge_bottom=bool(p.get("edge_bottom")),
            )
            for p in parts_data
        ]
        if not parts:
        ...
        meta = {
            "order_code": request.form["order_code"].strip()[:60], "customer": request.form.get("customer", "").strip()[:100],
            "material": request.form["material"].strip()[:100], "sheet_length": int(request.form["sheet_length"]),
            "sheet_width": int(request.form["sheet_width"]), "kerf": max(0, int(request.form["kerf"])),
            "trim": max(0, int(request.form["trim"])), "worker_name": request.form.get("worker_name", "").strip()[:100],
    ...
    flash(f"Kroy tayyor: {len(plans)} ta list. {tested_variants} ta variant tekshirildi, eng yaxshi taxlash tanlandi.")
...
def job_view(job_id: int) -> str:
    ...
    worker_url = url_for("worker_view", token=job["token"], _external=True)
    ...
    return render_page(
        ...
        edge_m=parts_edge_m(parts), worker_url=worker_url, qr=qr_data_uri(worker_url), usage=sheet_usage, offcut=offcut_text,
...
def worker_update(token: str, sheet_no: int) -> Response:
    ...
    stats = conn.execute("SELECT COUNT(*) total,SUM(cut_done) cuts,SUM(edge_done) edges FROM kroy_sheets WHERE job_id=?", (job["id"],)).fetchone()
    if stats and stats["total"] and stats["cuts"] == stats["total"] and stats["edges"] == stats["total"]:
        conn.execute("UPDATE kroy_jobs SET status='Tayyor',finished_at=? WHERE id=?", (now_iso(), job["id"]))
    elif stats and stats["cuts"]:
        conn.execute("UPDATE kroy_jobs SET status='Jarayonda' WHERE id=?", (job["id"],))
    conn.commit()
    conn.close()
...
@app.get("/api/health")
def health() -> Response:
    return jsonify({"ok": True, "module": "Mebel360 Kroy Secure V7"})


@app.errorhandler(413)
def too_large(_: Exception) -> tuple[str, int]:
    return "Fayl 5 MBdan katta", 413
